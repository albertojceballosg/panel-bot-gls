...
new #[Layout('components.layouts.app')] class extends Component
{
    ...
    /** Busca a la vez por nombre de ruta y por mensajero. */
    public string $search = '';

    /** Las dadas de baja se ocultan por defecto: son la excepción, no la norma. */
    ...
    {
        $pickupRoute = PickupRoute::withTrashed()->findOrFail($id);

        $this->editing = $pickupRoute->id;
        $this->name = $pickupRoute->name;
        $this->resetValidation();
        $this->showingForm = true;
    }
    ...
    {
        $this->reset('editing', 'name', 'showingForm');
        $this->resetValidation();
    }
    ...
    public function save(): void
    {
        ...
        try {
            // Cerrojo atómico en servidor: desactivar el botón no cubre el doble
            // clic que llega antes de que reaccione el JS, ni dos pestañas.
            $this->withoutDoubleSubmit('pickup-route:save', function () {
                // La transacción envuelve también la escritura del historial,
                // que se dispara en un evento del modelo: si algo falla, no
                // queda ni la fila ni un registro de auditoría huérfano (§4).
                DB::transaction(function () {
                    PickupRoute::withTrashed()
    ...
    {
        try {
            $this->withoutDoubleSubmit("pickup-route:delete:{$id}", function () use ($id) {
                DB::transaction(fn () => PickupRoute::findOrFail($id)->delete());
            });

            session()->flash('ok', 'Ruta dada de baja.');
        } catch (DoubleSubmitException) {
            return;
        } catch (\RuntimeException $e) {
            // El modelo se niega si la ruta todavía tiene comercios vivos (§4).
            // Mejor este mensaje que una excepción en pantalla.
    ...
    {
        try {
            $this->withoutDoubleSubmit("pickup-route:restore:{$id}", function () use ($id) {
                DB::transaction(fn () => PickupRoute::onlyTrashed()->findOrFail($id)->restore());
            });
        } catch (DoubleSubmitException) {
            return;
        }

        session()->flash('ok', 'Ruta reactivada.');
    }
}; ?>
                                    ...
                                    <div class="flex justify-end gap-1">
                                        ...
                                        @else
                                            ...
                                            <x-ui.icon-button label="Dar de baja" variant="danger"
                                                              wire:click="delete({{ $pickupRoute->id }})"
                                                              wire:loading.attr="disabled">
                                                <svg class="size-4" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
